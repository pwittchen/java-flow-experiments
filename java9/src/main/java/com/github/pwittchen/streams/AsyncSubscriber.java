...
/**
 * AsyncSubscriber is an implementation of Reactive Streams `Subscriber`, it runs asynchronously (on
 * an Executor), requests one element at a time, and invokes a user-defined method to process each
 * element.
 *
...
public abstract class AsyncSubscriber<T> implements Subscriber<T>, Runnable {
  ...
  // This is the Executor we'll use to be asynchronous, obeying rule 2.2
  ...
  private final void done() {
    //On this line we could add a guard against `!done`, but since rule 3.7 says that `Subscription.cancel()` is idempotent, we don't need to.
        ...
        true; // If `whenNext` throws an exception, let's consider ourselves done (not accepting more elements)
    if (subscription
        != null) { // If we are bailing out before we got a `Subscription` there's little need for cancelling it.
      try {
  ...
  private final void handleOnSubscribe(final Subscription s) {
    if (s == null) {
      // Getting a null `Subscription` here is not valid so lets just ignore it.
        ...
        != null) { // If someone has made a mistake and added this Subscriber multiple times, let's handle it gracefully
      try {
        s.cancel(); // Cancel the additional subscription to follow rule 2.5
      } catch (final Throwable t) {
        //Subscription.cancel is not allowed to throw an exception, according to rule 3.15
        (new IllegalStateException(
            s + " violated the Reactive Streams rule 3.15 by throwing an exception from cancel.",
            t)).printStackTrace(System.err);
      }
    } else {
      ...
      try {
        ...
        (new IllegalStateException(
            s + " violated the Reactive Streams rule 3.16 by throwing an exception from request.",
            t)).printStackTrace(System.err);
      }
    }
  }
  ...
  private final void handleOnNext(final T element) {
    if (!done) { // If we aren't already done
      if (subscription
          == null) { // Technically this check is not needed, since we are expecting Publishers to conform to the spec
          ...
          if (whenNext(element)) {
            try {
              subscription.request(
                  1); // Our Subscriber is unbuffered and modest, it requests one element at a time
            } catch (final Throwable t) {
              // Subscription.request is not allowed to throw according to rule 3.16
              (new IllegalStateException(subscription
        ...
        if (!done) { // If we're done, we shouldn't process any more signals, obeying rule 2.8
          ...
          } else if (s instanceof OnError) // We are always able to handle OnError, obeying rule 2.10
          {
            handleOnError(((OnError) s).error);
              ...
              == OnComplete.Instance) // We are always able to handle OnComplete, obeying rule 2.9
          {
            handleOnComplete();
          }
        }
